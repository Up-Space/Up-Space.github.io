...
export default function Search() {
  ...
  useEffect(() => {
    // This is a client component, so we can't directly call `getAllPosts`.
    // In a real application, you'd fetch this data from an API route.
    // For this demonstration, we'll use a placeholder.
    // To make this fully dynamic, you'd need to create a server component wrapper
    // to pass the data down as props.
    // For now, let's assume `allContent` is fetched.
    const fetchedContent = getAllPosts(); // This call will only work if you have a server component wrapper
    ...
    if (query) {
      setSearchTerm(query);
      performSearch(query, fetchedContent);
    }
  }, []);

  const performSearch = (term, data) => {
    setLoading(true);
    setTimeout(() => {
      if (!term.trim()) {
        setResults([]);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    performSearch(searchTerm, allContent);
